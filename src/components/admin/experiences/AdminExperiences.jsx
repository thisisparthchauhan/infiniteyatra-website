...
const AdminExperiences = () => {
    ...
    const handleUpdateStatus = async (storyId, newStatus) => {
        if (!window.confirm(`Are you sure you want to ${newStatus} this story?`)) return;
        ...
        try {
            ...
            await updateDoc(storyRef, {
                status: newStatus,
                reviewedAt: new Date(),
                // Note: In a real app, you'd add reviewedBy: currentUser.uid
            });
            ...
            if (selectedStory && selectedStory.id === storyId) {
                setSelectedStory(null); // Close modal on action
            }
        } catch (error) {
    ...
    const filteredStories = stories.filter(story => {
        ...
        return story.status === filter || (!story.status && filter === 'pending'); // Handle missing status as pending
    });
    ...
    // AI Insight Mockup Component (Dark Theme)
    const AiInsightPanel = ({ story }) => (
        <div className="bg-gradient-to-br from-indigo-900/20 to-purple-900/20 rounded-xl p-5 border border-indigo-500/20 mb-6">
            <div className="flex items-center gap-2 mb-3">
                <div className="w-8 h-8 rounded-full bg-indigo-500/20 flex items-center justify-center">
                    <Shield size={16} className="text-indigo-400" />
                </div>
                <h4 className="font-bold text-indigo-300">AI Safety & Quality Insight</h4>
            </div>

            <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                    <span className="block text-indigo-400/60 text-xs font-semibold uppercase tracking-wider mb-1">Emotional Tone</span>
                    <span className="text-indigo-100 font-medium">✨ Reflective & Inspiring</span>
                </div>
                <div>
                    <span className="block text-indigo-400/60 text-xs font-semibold uppercase tracking-wider mb-1">Safety Check</span>
                    <span className="text-green-400 font-medium flex items-center gap-1">
                        <Check size={14} /> No PII or Content Flags
                    </span>
                </div>
                <div className="col-span-2">
                    <span className="block text-indigo-400/60 text-xs font-semibold uppercase tracking-wider mb-1">Suggested 1-Line Preview</span>
                    <p className="text-indigo-200/80 italic">"{story.description.slice(0, 100)}..."</p>
                </div>
            </div>
        </div>
    );
    ...
    return (
        <div className="p-6">
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h1 className="text-2xl font-bold text-white">Experience Review</h1>
                    <p className="text-slate-400">Review, approve, or reject user submitted experiences</p>
                </div>
